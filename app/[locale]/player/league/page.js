...
export default function PlayerLeague() {
  ...
  const tabs = getLeagueTabs(locale)
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          {locale === 'es' ? 'No tienes liga asignada' : 'No league assigned'}
        </h3>
        ...
        <button
          onClick={() => router.push(`/${locale}/${locale === 'es' ? 'ligas' : 'leagues'}`)}
        ...
        >
          {locale === 'es' ? 'Explorar ligas disponibles' : 'Explore available leagues'}
        </button>
      </div>
    )
  }
            ...
            <div className="flex items-center space-x-4">
              <div>
                ...
                <p className="text-purple-100 mt-1">
                  {locale === 'es' ? 'Temporada' : 'Season'}: {player.season}
                </p>
                <p className="text-purple-200 text-sm mt-1">
                  {locale === 'es' ? 'Tu ELO' : 'Your ELO'}: {player.stats?.eloRating || 1200}
                </p>
              </div>
            </div>
          </div>
