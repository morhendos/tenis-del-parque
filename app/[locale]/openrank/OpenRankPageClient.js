      ...
      <section className="pt-24 pb-12 px-4">
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl shadow-lg shadow-amber-500/30 mb-6">
              <svg className="w-10 h-10 text-white" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" />
              </svg>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">{t.title}</h1>
            <p className="text-xl text-amber-600 font-medium mb-4">{t.subtitle}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-200 text-center">
              <div className="text-4xl font-bold text-amber-900 mb-1">{stats.qualifiedCount}</div>
              <div className="text-sm text-amber-700">{t.qualifiedPlayers}</div>
            </div>
            <div className="bg-gradient-to-br from-gray-50 to-slate-50 rounded-2xl p-6 border border-gray-200 text-center">
              <div className="text-4xl font-bold text-gray-900 mb-1">{stats.totalPlayers}</div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl p-6 border border-purple-200 text-center">
              <div className="text-4xl font-bold text-purple-900 mb-1">{stats.matchesRequired}</div>
              <div className="text-sm text-purple-700">{t.matchesRequired}</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-4xl">
          {loading ? (
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-8">
              ...
              <div className="divide-y divide-gray-100">
                {rankings.map((player) => (
                  <div 
                    key={player._id}
                    className={`px-6 py-4 transition-colors ${getPositionRowStyle(player.position)}`}
                  >
                    <div className="grid grid-cols-12 gap-2 items-center">
                      {/* Position */}
                      <div className="col-span-1 flex justify-center">
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${getPositionBadgeStyle(player.position)}`}>
                          {player.position}
                        </span>
                      </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center mb-8">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">{t.noRankings}</h3>
              <p className="text-gray-500 mb-6">{t.noRankingsDesc}</p>
              <Link 
                href={`/${locale}/ligas`}
                className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-semibold hover:from-amber-600 hover:to-orange-600 transition-all shadow-lg"
              >
                {t.joinNow}
              </Link>
            </div>
                ...
                <div className="bg-gray-50 rounded-2xl border border-gray-200 overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-100">
                    {almostQualified.slice(0, 10).map((player) => (
                      <div key={player._id} className="px-6 py-4">
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-amber-50 to-orange-50">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {t.howItWorksItems.map((item, index) => (
              <div key={index} className="bg-white rounded-xl p-6 shadow-md text-center">
                <div className="w-12 h-12 bg-gradient-to-br from-amber-400 to-orange-500 text-white rounded-full flex items-center justify-center text-xl font-bold mx-auto mb-4">
                  {index + 1}
                </div>
          ...
          <div className="text-center mt-12 space-x-4">
            <Link 
              href={`/${locale}/elo`}
              className="inline-flex items-center px-6 py-3 bg-white text-amber-700 border-2 border-amber-500 rounded-xl font-semibold hover:bg-amber-50 transition-all"
            >
