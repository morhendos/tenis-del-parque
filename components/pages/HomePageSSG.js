...
export default function HomePageSSG({ locale, leaguesData }) {
  ...
  const { leagues, currentSeasonLeagues, comingSoonLeagues, total, error } = leaguesData;
  
  // Group leagues by season for unified display
  // Current seasons include both registration_open AND active leagues
  const currentSeasonGroups = groupLeaguesBySeason(currentSeasonLeagues)
          ...
          <div className="relative">
            {error ? (
              ...
              <div className="space-y-8">
                {/* PRIORITY 1: Current Season Leagues (Registration Open + Active) */}
                {currentSeasonGroups.length > 0 && (
                  <div>
                    {currentSeasonGroups.map((group, index) => {
                      const cityName = group.leagues[0]?.city?.name?.[validLocale] || group.leagues[0]?.city?.name?.es || ''
                      // Check if any league in this group is registration open
                      const hasRegistrationOpen = group.leagues.some(l => l.status === 'registration_open')
                      
                      return (
                        <div key={`current-${index}`} className="mb-6 last:mb-0">
                          <h3 className="text-[0.9rem] sm:text-lg font-medium text-gray-600 mb-4 text-center flex items-center justify-center gap-2">
                            <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${hasRegistrationOpen ? 'bg-parque-green' : 'bg-blue-600'}`} />
                            {cityName}
                          </h3>
