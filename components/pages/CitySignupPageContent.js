...
export default function CitySignupPageContent({ locale }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!league) {
      setErrors({ submit: locale === 'es' ? 'Información de la liga no disponible' : 'League information not available' })
      return
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (response.status === 409) {
          ...
          // Generic error
          setErrors({ submit: data.error || (locale === 'es' ? 'Algo salió mal. Por favor, inténtalo de nuevo.' : 'Something went wrong. Please try again.') })
        }
      }
    } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-b from-parque-bg via-white to-white">
        ...
        <div className="container mx-auto px-4 py-32 text-center">
          <h1 className="text-4xl font-light text-parque-purple mb-4">
            {locale === 'es' ? 'Liga no encontrada' : 'League not found'}
          </h1>
  ...
  const signupContent = {
    ...t.signup,
    title: isComingSoon 
      ? (locale === 'es' ? `Pre-regístrate para ${league.name}` : `Pre-register for ${league.name}`)
      : (locale === 'es' ? `Únete a ${league.name}` : `Join the ${league.name} League`),
    subtitle: isComingSoon
      ? (locale === 'es' ? `Sé de los primeros en jugar cuando lancemos en ${league.name}` : `Be among the first to play when we launch in ${league.name}`)
      : (locale === 'es' ? `Juega al tenis en ${league.name} con jugadores de tu nivel` : `Play tennis in ${league.name} with players at your level`)
  }
