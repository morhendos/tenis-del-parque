...
const MatchSchema = new mongoose.Schema({
  ...
  // BYE match flag - when true, player2 is null and player1 gets automatic win
  isBye: {
    type: Boolean,
    default: false
  },
  ...
  players: {
    ...
    player2: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'Player',
      required: function() { return !this.isBye; } // Not required for BYE matches
    }
  },
  ...
  schedule: {
    ...
    // Track who extended the deadline and when
    extensionHistory: [{
      player: {
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Player'
...
MatchSchema.methods.calculatePoints = function() {
  // BYE matches give 3 points to player1, 0 to player2 (which doesn't exist)
  if (this.isBye) {
...
MatchSchema.pre('save', function(next) {
  // Skip player validation for BYE matches
  if (!this.isBye) {
    // Ensure players are different for regular matches
    if (this.players.player2 && this.players.player1.equals(this.players.player2)) {
  ...
  if (this.status === 'completed') {
    ...
    // For BYE matches, winner must be player1
    if (this.isBye) {
      const winnerId = this.result.winner.toString();
      const player1Id = this.players.player1.toString();
      
      if (winnerId !== player1Id) {
        next(new Error('BYE match winner must be player1'));
        return;
      }
    } else {
      // For regular matches, ensure winner is one of the players
      const winnerId = this.result.winner.toString();
      const player1Id = this.players.player1.toString();
