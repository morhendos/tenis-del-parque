...
CitySchema.statics.findOrCreate = async function(cityData) {
  ...
  if (!city) {
    ...
    city = await this.create({
      slug: cityData.slug,
      name: {
        es: cityData.nameEs || cityData.name || cityData.slug.charAt(0).toUpperCase() + cityData.slug.slice(1),
        en: cityData.nameEn || cityData.name || cityData.slug.charAt(0).toUpperCase() + cityData.slug.slice(1)
      },
