...
const LeagueSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['active', 'inactive', 'coming_soon', 'registration_open', 'completed', 'archived'],
    default: 'coming_soon'
  },
...
LeagueSchema.statics.createNewSeason = async function(baseLeagueId, seasonData) {
  ...
  const newLeague = new this({
    ...
    // Copy playoff STRUCTURE settings but RESET runtime data
    // This prevents inheriting qualified players from previous seasons
    playoffConfig: seasonData.playoffConfig || {
      enabled: baseLeague.playoffConfig?.enabled ?? true,
      numberOfGroups: baseLeague.playoffConfig?.numberOfGroups ?? 1,
      groupAPlayers: baseLeague.playoffConfig?.groupAPlayers ?? 8,
      groupBPlayers: baseLeague.playoffConfig?.groupBPlayers ?? 8,
      format: baseLeague.playoffConfig?.format ?? 'tournament',
      // ALWAYS reset runtime data for new seasons
      currentPhase: 'regular_season',
      playoffStartDate: null,
      playoffEndDate: null,
      qualifiedPlayers: { groupA: [], groupB: [] },
      bracket: { groupA: null, groupB: null }
    }
  })
