...
export const playoffEmailTemplate = {
  es: {
    subject: '{stageSubject} — {leagueName}',
...
<tr><td style="padding: 32px;">
  ...
  <div style="border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden; margin-bottom: 24px;">
    <div style="background: #f9fafb; padding: 12px 16px; border-bottom: 1px solid #e5e7eb;">
      <strong style="color: #1f2937; font-size: 14px;">{stageName}</strong>
    </div>
...
export function generatePlayoffEmail(data) {
  const {
    playerName, playerEmail, language = 'es', position, points, seed,
    playoffGroup, topPlayers, leagueName, opponentName, opponentSeed,
    opponentWhatsApp, opponentMatches, opponentPoints, semifinalMatchup,
    bracketUrl, dashboardUrl, stage = 'quarterfinal'
  } = data
  ...
  const stages = {
    quarterfinal: {
      es: { name: 'Cuartos de Final', subject: 'Playoffs - Cuartos de Final', intro: 'Has clasificado para la fase de playoffs. A continuacion tienes toda la informacion sobre tu partido de cuartos de final.' },
      en: { name: 'Quarterfinal', subject: 'Playoffs - Quarterfinal', intro: 'You have qualified for the playoff stage. Below you will find all the details about your quarterfinal match.' }
    },
    semifinal: {
      es: { name: 'Semifinal', subject: 'Playoffs - Semifinal', intro: 'Felicidades por avanzar a las semifinales. Aqui tienes los detalles de tu proximo partido.' },
      en: { name: 'Semifinal', subject: 'Playoffs - Semifinal', intro: 'Congratulations on advancing to the semifinals. Here are the details of your next match.' }
    },
    final: {
      es: { name: 'Final', subject: 'Playoffs - Final', intro: 'Has llegado a la gran final. Aqui tienes los detalles del partido mas importante de la temporada.' },
      en: { name: 'Final', subject: 'Playoffs - Final', intro: 'You have reached the grand final. Here are the details of the most important match of the season.' }
    },
    third_place: {
      es: { name: 'Tercer Puesto', subject: 'Playoffs - Tercer Puesto', intro: 'Aqui tienes los detalles del partido por el tercer puesto.' },
      en: { name: '3rd Place', subject: 'Playoffs - 3rd Place', intro: 'Here are the details of the 3rd place match.' }
    }
  }

  const sc = stages[stage]?.[language] || stages.quarterfinal[language] || stages.quarterfinal.es
  const whatsappMsg = language === 'es'
    ? `Hola ${opponentName}, soy ${playerName}. Nos toca en ${sc.name.toLowerCase()} de los playoffs. Cuando te vendria bien? Un saludo`
    : `Hi ${opponentName}, this is ${playerName}. We are matched for the playoff ${sc.name.toLowerCase()}. When would suit you? Cheers`
  ...
  const replacements = {
    ...
    '{whatsappMessage}': encodeURIComponent(whatsappMsg),
    '{stageName}': sc.name,
    '{stageSubject}': sc.subject,
    '{stageIntro}': sc.intro
  }
  ...
  return { subject: emailSubject, html: emailHtml, text: emailSubject }
}
