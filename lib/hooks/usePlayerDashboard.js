...
export function usePlayerDashboard() {
  ...
  const fetchPlayerData = useCallback(async () => {
    try {
      ...
      let profileResponse = await fetch('/api/player/profile')
      
      // Retry once on 401 — session cookie may not have propagated yet after login
      ...
      if (profileResponse.ok) {
        ...
        if (profileData.player) {
          ...
          let matchesResponse = await fetch('/api/player/matches')
          if (!matchesResponse.ok) {
            console.log(`[Dashboard] Matches returned ${matchesResponse.status}, retrying after 1.5s...`)
            await new Promise(r => setTimeout(r, 1500))
            matchesResponse = await fetch('/api/player/matches')
          }
          if (!matchesResponse.ok) {
            console.log(`[Dashboard] Matches retry 1 failed (${matchesResponse.status}), retrying after 3s...`)
