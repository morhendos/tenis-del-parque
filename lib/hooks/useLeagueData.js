  ...
  const fetchPlayerAndLeague = useCallback(async () => {
    try {
      ...
      if (playerData.player?.league) {
        ...
        const matchesRes = await fetch(`/api/leagues/${leagueId}/matches?season=${dbSeasonName}&status=completed&limit=100`)
        if (matchesRes.ok) {
          const matchesData = await matchesRes.json()
          setMatches(matchesData.matches || [])
        }
        
        const scheduleRes = await fetch(`/api/leagues/${leagueId}/matches?season=${dbSeasonName}&status=scheduled&limit=50`)
        if (scheduleRes.ok) {
