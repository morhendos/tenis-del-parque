...
export function useLeagueData() {
  ...
  const seasonToDbName = (seasonName) => {
    const mapping = {
      ...
      'Verano 2026': 'summer-2026',
      'Invierno 2026': 'winter-2026',
      'Primavera 2026': 'spring-2026',
      'Otoño 2026': 'autumn-2026'
    }
  ...
  const fetchPlayerAndLeague = useCallback(async () => {
    try {
      ...
      const playerRes = await fetchWithRetry('/api/player/profile', {
        credentials: 'include'
      ...
      if (selectedLeague) {
        ...
        const standingsRes = await fetchWithRetry(`/api/leagues/${leagueId}/standings?season=${dbSeasonName}`, {}, { label: 'League:standings' })
        if (standingsRes.ok) {
          const standingsData = await standingsRes.json()
          setStandings(standingsData)
        }
        
        const matchesRes = await fetchWithRetry(`/api/leagues/${leagueId}/matches?season=${dbSeasonName}&status=completed&limit=200`, {}, { label: 'League:matches' })
        if (matchesRes.ok) {
          const matchesData = await matchesRes.json()
          setMatches(matchesData.matches || [])
        }
        
        const scheduleRes = await fetchWithRetry(`/api/leagues/${leagueId}/matches?season=${dbSeasonName}&limit=200`, {}, { label: 'League:schedule' })
        if (scheduleRes.ok) {
