...
export function calculatePlayerStats(playerId, matches) {
  ...
  // Calculate games from BYE matches (they have actual scores stored: 6-0, 6-0)
  let byeGamesWon = 0
  let byeGamesLost = 0
  byeMatches.forEach(match => {
    if (match.result?.score?.sets) {
      match.result.score.sets.forEach(set => {
        byeGamesWon += set.player1 || 0
        byeGamesLost += set.player2 || 0
      })
    } else {
      // Fallback: assume 6-0, 6-0 = 12 games won
      byeGamesWon += 12
    }
  })
  ...
  const stats = {
    ...
    setsLost: 0,
    gamesWon: byeGamesWon, // Games from BYE matches (6-0, 6-0 = 12)
    gamesLost: byeGamesLost,
