 ...
 * options.preferenceKey: only send to subscriptions where preferences[key] !== false
 */
export async function sendToPlayers(playerIds, notification, options = {}) {
  ensureVapidConfigured()
  await dbConnect()
  ...
  if (options.preferenceKey) {
    query[`preferences.${options.preferenceKey}`] = { $ne: false }
  }
 ...
 * options.excludePlayerIds: players to skip (e.g. the two who played the match)
 * options.preferenceKey: forwarded to sendToPlayers
 */
export async function sendToLeague(leagueId, notification, options = {}) {
  ensureVapidConfigured()
  await dbConnect()
  ...
  if (options.excludePlayerIds && options.excludePlayerIds.length > 0) {
    const exclude = new Set(options.excludePlayerIds.map(id => String(id)))
    playerIds = playerIds.filter(id => !exclude.has(String(id)))
  }
  ...
  return sendToPlayers(playerIds, notification, options)
}

/**
 * Broadcast a completed match result to everyone in the league
 * (except the two players who played it). Never throws.
 */
export async function broadcastMatchResult(matchId) {
  try {
    await dbConnect()
    const Match = (await import('@/lib/models/Match')).default
    const { formatPlayerNameForStandings } = await import('@/lib/utils/playerNameUtils')
    
    const match = await Match.findById(matchId)
      .populate('players.player1 players.player2', 'name')
      .lean()
    
    if (!match || match.status !== 'completed' || !match.result?.winner) return
    
    const p1 = match.players?.player1
    const p2 = match.players?.player2
    if (!p1 || !p2 || !match.league) return
    
    const winnerIsP1 = String(match.result.winner) === String(p1._id)
    const winnerName = formatPlayerNameForStandings((winnerIsP1 ? p1 : p2).name, 'es')
    const loserName = formatPlayerNameForStandings((winnerIsP1 ? p2 : p1).name, 'es')
    ...
    if (match.result.score?.walkover) {
      ...
      score = sets
        .map(s => winnerIsP1 ? `${s.player1}-${s.player2}` : `${s.player2}-${s.player1}`)
        .join(', ')
    }
    
    const notification = {
      title: '\uD83C\uDFBE Nuevo resultado',
      body: `${winnerName} gan\u00f3 a ${loserName} ${score}`.trim(),
      tag: `league-result-${match._id}`,
      url: '/player/league?tab=matches',
      data: { matchId: String(match._id) }
    }
    
    return await sendToLeague(match.league, notification, {
      excludePlayerIds: [p1._id, p2._id],
      preferenceKey: 'matchResult'
    })
  } catch (error) {
    console.error('broadcastMatchResult failed:', error.message)
  }
}
