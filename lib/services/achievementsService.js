...
export function getPlayoffTrophies(league, playerId) {
  ...
  // Check if Champion
  const finalWinner = bracket.final?.winner?.toString()
  ...
  if (bracket.final?.matchId && finalWinner && finalWinner !== playerIdStr) {
    const sfWinners = bracket.semifinals?.map(sf => sf.winner?.toString()).filter(Boolean) || []
  ...
  // Check if Semi-finalist (lost in semifinals)
  if (bracket.semifinals && bracket.quarterfinals) {
    const sfWinners = bracket.semifinals.map(sf => sf.winner?.toString()).filter(Boolean)
    const qfWinners = bracket.quarterfinals.map(qf => qf.winner?.toString()).filter(Boolean)
