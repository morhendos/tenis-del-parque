...
async function resolveAudience(audience) {
  ...
  switch (audience.type) {
    ...
    case 'league_non_playoff': {
      if (!audience.leagueId) throw new Error('leagueId required for league_non_playoff audience')
      
      const lpLeagueId = new mongoose.Types.ObjectId(audience.leagueId)
      ...
      // Get qualified player IDs from playoff config
      const lpLeague = await League.findById(audience.leagueId, 'playoffConfig').lean()
      const qualifiedIds = new Set()
      if (lpLeague?.playoffConfig?.qualifiedPlayers?.groupA) {
        lpLeague.playoffConfig.qualifiedPlayers.groupA.forEach(qp => qualifiedIds.add(qp.player.toString()))
      }
      if (lpLeague?.playoffConfig?.qualifiedPlayers?.groupB) {
        lpLeague.playoffConfig.qualifiedPlayers.groupB.forEach(qp => qualifiedIds.add(qp.player.toString()))
      }
      
      // Filter out qualified players
      const nonPlayoff = allPlayers.filter(p => !qualifiedIds.has(p._id.toString()))
      console.log(`[Audience] League non-playoff: ${allPlayers.length} total, ${qualifiedIds.size} qualified, ${nonPlayoff.length} non-playoff`)
      
      return deduplicatePlayers(nonPlayoff)
    }
...
// Strip emojis from text (for email body - keep emojis only in subject line)
...
function buildEmailHtml(subject, body, locale) {
  if (!locale) locale = 'es'
  var dashboardUrl = (process.env.NEXTAUTH_URL || 'https://www.tenisdp.es') + '/' + locale + '/player/dashboard'
  var ctaLabel = locale === 'es' ? 'Ir a mi Dashboard' : 'Go to Dashboard'
  var logoUrl = 'https://www.tenisdp.es/logo-horizontal-small.png'
  ...
  return '<!DOCTYPE html>'
    ...
    + '<img src="' + logoUrl + '" alt="Tenis del Parque" style="height: 64px; width: auto;" />'
    + '</td></tr>'
    ...
    + '<h1 style="color: white; margin: 0; font-size: 20px; font-weight: 700; letter-spacing: 0.3px;">' + stripEmojis(subject) + '</h1>'
    + '</td></tr>'
    // Content
    + '<tr><td style="padding: 32px;">'
    + '<div style="color: #4b5563; font-size: 15px; line-height: 1.7;">' + bodyHtml + '</div>'
...
export async function sendNotification({ audience, channels, message, language = 'auto', sender }) {
  ...
  console.log('[MessageCenter] Sending to ' + players.length + ' players via ' + Object.entries(channels).filter(function(e) { return e[1] }).map(function(e) { return e[0] }).join(' + ') + ' (language: ' + language + ')')
  ...
  for (const player of players) {
    ...
    if (channels.email && player.email) {
      ...
      if (stats.emailsSent > 0 || stats.emailsFailed > 0) {
 ...
 */
export async function getMessageHistory(limit) {
  if (!limit) limit = 20
  await dbConnect()
