...
function buildEmailHtml(subject, body, locale = 'es') {
  ...
  const logoUrl = 'https://www.tenisdp.es/logo-horizontal-small.png'
  ...
  return [
    ...
    '<tr><td style="text-align: center; padding: 16px 0 20px 0;">',
    ...
    // Card with rounded corners starts here
    ...
    '<div style="color: #4b5563; font-size: 15px; line-height: 1.7;">' + body.replace(/\n/g, '<br>') + '</div>',
...
}ified Notification Service for Tenis del Parque
 * 
 * Orchestrates sending messages via multiple channels (email + push).
 * Handles audience resolution (all players, league, round, individual).
 * Supports bilingual messages (auto per player preference, or forced ES/EN).
