...
export async function updatePlayerStatsOnMatchComplete(match, player1, player2, session = null) {
  ...
  if (!player1Reg.stats) player1Reg.stats = { matchesPlayed: 0, matchesWon: 0, setsWon: 0, setsLost: 0, totalPoints: 0 }
  if (!player2Reg.stats) player2Reg.stats = { matchesPlayed: 0, matchesWon: 0, setsWon: 0, setsLost: 0, totalPoints: 0 }

  // Calculate points (3 for win, 1 for loss, 0 for walkover loss)
  const player1Points = player1Won ? 3 : (match.result.score?.walkover ? 0 : 1)
  const player2Points = player2Won ? 3 : (match.result.score?.walkover ? 0 : 1)
